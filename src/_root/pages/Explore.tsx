  ...
  return (
    <div className=" explore-container">
      <div className="explore-inner_container">
        <h2 className=" h3-bold w-full">Search Post</h2>
      ...
      <div className="flex flex-wrap gap-9 w-full max-w-5xl">
        {shouldShowSearchResults ? (
          <SearchResults
            ...
            searchPosts={searchPosts?.documents} // Adjusted to ensure documents is always an array
          />
        ) : shouldShowPost ? (
          <p className=" text-light-4 mt-4 text-center w-full">End of posts</p>
        ) : (
          posts?.pages.map((item, index) => (
            <GridPostList key={`page-${index}`} posts={item?.documents || []} /> // Adjusted to ensure documents is always an array
          ))
        )}
      </div>
